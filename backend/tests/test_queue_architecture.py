...
async def test_queue_job_creation():
    ...
    with patch("services.queue_manager.message_queue.add", new_callable=AsyncMock) as mock_add:
        # Action
...
async def test_worker_processing_success(mock_job):
    ...
    with patch("worker_main.WhatsAppService.send_template_message", new_callable=AsyncMock) as mock_wa, \
         patch("worker_main._db_recipients.update_status") as mock_db, \
         ...
         patch("worker_main.get_settings", return_value={"phone_number_id": "1", "access_token": "2"}), \
         patch("worker_main._db_messages.add"):
        ...
        mock_db.assert_any_call("camp1", "919876543210", "processing")
        mock_db.assert_any_call("camp1", "919876543210", "sent", wa_message_id="wamid_123")
...
async def test_retry_mechanism_and_failure(mock_job):
    ...
    with patch("worker_main.WhatsAppService.send_template_message", new_callable=AsyncMock) as mock_wa, \
        ...
        mock_db.assert_any_call("camp1", "919876543210", "retrying", error_message="WhatsApp API Error: (#131056) Rate limit hit")
...
async def test_failure_handling_dead_letter(mock_job):
    ...
    with patch("worker_main.enqueue_dead_letter", new_callable=AsyncMock) as mock_dlq, \
         patch("worker_main._db_recipients.update_status") as mock_db, \
         patch("worker_main._db_counters.increment"):
        
        await on_failed_message(mock_job, Exception("Timeout Error"))
        
        mock_db.assert_any_call("camp1", "919876543210", "failed", error_message="Exhausted retries: Timeout Error")
...
async def test_network_failures_recovery(mock_job):
    ...
    with patch("worker_main.get_settings", return_value={"phone_number_id": "1", "access_token": "2"}), \
        ...
        mock_db.assert_any_call("camp1", "919876543210", "retrying", error_message="Network Unreachable")
